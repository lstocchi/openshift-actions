...
suite('OcAuth', () => {
    ...
    suite('initOpenShiftEndpoint', () => {
        test('reject promise if server is not valid', async () => {
            try {
                await OcAuth.initOpenShiftEndpoint(null, 'params');
                expect.fail();
            } catch (err) {
                expect(err).equals('Invalid Openshift cluster URL.');
            }
        });

        test('reject promise if there are no parameters', async () => {
            try {
                await OcAuth.initOpenShiftEndpoint('server', '');
                expect.fail();
            } catch (err) {
                expect(err).equals('Invalid parameters workflow input.');
            }
        });
        ...
        test('check if correct endpoint is created if scheme is basic_authentication', async () => {
            ...
            const res = await OcAuth.initOpenShiftEndpoint('server', params);
        ...
        test('reject promise if there are no sufficient params for authentication', async () => {
            ...
            try {
                await OcAuth.initOpenShiftEndpoint('server', params);
                expect.fail();
            } catch (err) {
                expect(err).equals('There are no sufficient parameters to authenticate to an Openshift cluster.');
            }            
        });
    });

    suite('createKubeConfig', () => {
        ...
        test('reject promise if endpoint has no a value', async () =>  {
            try {
                await OcAuth.createKubeConfig(null, 'path');
                expect.fail();
            } catch (err) {
        ...
        test('check if getCertificateAuthority is called with correct endpoint', async () => {
            ...
            await OcAuth.createKubeConfig(endpoint, 'path');
        ...
        test('check if new error is thrown if no vail authentication type is found', async () => {
            ...
            try {
                await createKubeConfigSpy(endpoint, 'path');
            } catch (err) {}
