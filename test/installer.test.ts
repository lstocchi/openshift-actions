...
suite('Installer', () => {
    ...
    suite('installOc', () => {
        ...
        test('error if no valid oc download url is found', async () => {
            sandbox.stub(Installer, 'getOcBundleUrl').resolves(undefined);
            try {
                await Installer.installOc('3.11', 'OS');
                expect.fail();
            } catch (err) {
        ...
        test('error if no valid oc binary is found', async () => {
            sandbox.stub(Installer, 'getOcBundleUrl').resolves('url');
            sandbox.stub(Installer, 'downloadAndExtract').resolves(undefined);
            try {
                await Installer.installOc('3.11', 'OS');
                expect.fail();
            } catch (err) {
    ...
    suite('downloadAndExtract', () => {
        test('retrun null if url is not valid', async () => {
